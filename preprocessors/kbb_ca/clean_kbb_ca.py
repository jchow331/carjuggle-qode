...
logger_name = 'kbb_ca_cleaner_bots_canada'
email_subject = 'KBB CA Cleaner Bot Alert'
...
def import_files(RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = {"_id": "id", "_VIN":'vin'}, inplace = True)

  
    df['id']=np.where(df['vin'].isnull(), df['url'].astype(str).str.split('=').str[1], df['vin'])
...
def initial_formatting(df, dfcleaned):
    ...
    ########
    df.drop(df[df["Vehicle_information.exterior_colour"].isnull()].index,inplace=True)
    ######
    ...
    df.loc[df[df['make'].str.contains('New', na=False)].index, 'condition']='New'
    df.loc[df[(~df['make'].str.contains('New', na=False)) & (df['condition'].isnull()) &(~df['kilometers'].isnull()) & (df['kilometers']> 10000)].index, 'condition']='Used'        

    #calculate the median for new cars
    median_kilometers=df.loc[df[(df["condition"]=='New')&(~df['kilometers'].isnull())].index, 'kilometers'].median()
    #print(median_kilometers)
        

    #find null values and substitute them with the median kilometers value
    df.loc[df[(df["condition"]=='New')&(df['kilometers'].isnull())].index,'kilometers']=median_kilometers

    df.drop(df[df['kilometers'].isnull()].index,inplace=True)
    df.reset_index(drop=True, inplace=True) 

    # transform kilomters to int
    ...
    # Remove 'New' from the year column and keep the next four digits  or the next word
    df['year'].replace(to_replace='New ', value='', regex=True, inplace=True)
    df['year'] = [x[0] for x in df['year'].str.split()]
...
def clean_makes(df,trims_kbb):
    ...
    df['make'].replace(to_replace='New ', value='', regex=True, inplace=True)
    df['make'] = [listToString(x[-1:]) for x in df['make'].str.split()]
...
# # =============================================================================
    ...
    df['province'].replace(to_replace='Edson, AL T7E 1V8', value='Edson, AB T7E 1V8', regex=True,inplace=True)
    df['province'].replace(to_replace='Bolton, L7E 1M3', value='Bolton, ON L7E 1M3', regex=True,inplace=True)
    df['province'].replace(to_replace='Bolton,  L7E 1M3', value='Bolton, ON L7E 1M3', regex=True,inplace=True)
    df['province'].replace(to_replace='Trenton, OH K8V 5R1', value='Trenton, ON K8V 5R1', regex=True,inplace=True)
    df['province'].replace(to_replace=',,', value=',', regex=True,inplace=True)
...
def clean_drivetrain(df):
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace=r'.*Rear Wheel Drive.*', value='FWD', regex=True, inplace=True)
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace=r'.*4 wheel drive*', value='4WD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace=r'.*4x4.*', value='4WD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace=r'.*4x4.*', value='4WD', regex=True, inplace=True)
...
def clean_numowners_previousaccident(df):
    logger.info(f"Cleaning owners and previous accidents...")
    ...
    logger.info(f"Cleaning owners and previous accidents... Done")
    logger.info(f"size is  {df.shape}")
...
def export_files(DF1, link1):
    logger.info(f"Exporting files... Done")
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/kbbCA/kbbCA_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/kbbCA/kbbCA_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/kbbCA/decoded.csv"
    ...
    #PROGRESS_PATH = 'MasterCode1/cleaning/kbbCA/cleaned_kbbCA_progress.csv'
    EXPORT_PATH_TIMELESS = 'MasterCode1/cleaning/kbbCA/cleaned_kbbCA_timeless.csv'
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/kbbCA/cleaned_kbbCA_time.csv'
