...
logger_name = 'canadadrives_cleaner_bots_canada'
email_subject = 'Canadadrives Cleaner Bot Alert'
...
def get_ext_color(x):
    try:
        if 'Exterior Colour' in x.keys():
            return x['Exterior Colour']
        else:
            return ""
    except:
        return ""
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = { 'scraped_first_time':'date_added', 'city':'City','Vehicle_information.drive_train':'Vehicle_information.drivetrain'}, inplace = True)

    #print('Display recent results')
    pd.set_option('display.max_columns', None)
    #print(df)

    if ('vin' in df.columns) and ('_VIN' in df.columns):
        df['vin'] = np.where(df['vin'].isnull(), df['_VIN'], df['vin'])
    df['vin']=np.where(df['vin'].isnull(), df['metadata'].str[0].replace('"', ''), df['vin'])

    if ('condition' in df.columns) and ('Condition' in df.columns):
        df['condition'] = np.where(df['condition'].isnull(), df['Condition'], df['condition'])
        
        
    if ('id' in df.columns) and ('_id' in df.columns):
        df['id'] = np.where(df['id'].isnull(), df['_id'], df['id'])
    ...
    if not(df.columns.is_unique):
        mask=[list(df.columns.duplicated())]
        col_ndex_to_change = np.array(range(len(df.columns)))[mask]
        columns_name=df.columns[col_ndex_to_change ][0]
        df.columns.values[col_ndex_to_change ]=columns_name+'_x'
        df[columns_name]=np.where(df[columns_name].isnull(), df[columns_name+'_x'],df[columns_name])
        df.drop(columns_name+'_x', axis=1,inplace=True)
        
        
    df['City'] = np.where(((df['City'].isnull())& (df['province']=='AB')), 'Airdrie', df['City'])
    df['City'] = np.where(((df['City'].isnull()) & ( df['province']=='BC')), 'Richmond', df['City'])
    df['City'] = np.where(((df['City'].isnull()) & (df['province']=='ON')), 'Brampton', df['City'])
...
#     df['metadata'] = df['metadata'].swifter.progress_bar(False).apply(lambda x: str_to_dict(x) )
#     df['ext_colour']=df['metadata'].swifter.progress_bar(False).apply(lambda x: get_ext_color(x))
#     df['Vehicle_information.exterior_colour'] = np.where(df['Vehicle_information.exterior_colour'].isnull(), df['ext_colour'], df['Vehicle_information.exterior_colour'])
...
#Turns km and prices into ints
    df['kilometers'] = df['kilometers'].astype(str)
    ...
    df['kilometers'].replace(to_replace=r' km', value='', regex=True, inplace=True)
    df.drop(df[(df["kilometers"].isnull())|(df["kilometers"].str.strip()=="")|(df['kilometers'].str.contains('\$', na=False, regex=True))].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    df['kilometers'] = df['kilometers'].astype(float).astype(int)



    df['price'].replace(to_replace=r',', value='', regex=True, inplace=True)
    df['price'].replace(to_replace=r'\$', value='', regex=True, inplace=True)
    df.drop(df[(df["price"].isnull())|(df["price"]=="")|(df["price"]=="Contact for price")|(df["price"]=="Get price alert")].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
...
def clean_trims(df, trims_kbb):
    ...
    # assign empty text to missing trim values 
    df.loc[df[(df['Vehicle_information.trim']=='0')|(df['Vehicle_information.trim']==0)|(df['Vehicle_information.trim'].isnull())].index,'Vehicle_information.trim'] = ''
    df['together']= df.Series.astype(str).str.cat(df.Trim.astype(str), sep=' ')
    df['together'] = df['together'].str.title()
    df['Vehicle_information.trim'] = np.where(df['Vehicle_information.trim']=='', df['together'], df['Vehicle_information.trim'])
...
def get_decoder(df, DECODED_PATH):
    ...
    try:
        ...
        VIN_Missing=list(set(df['id'])-set(decoder['vin'])-set(df['url']))
        ...
        try:
            ...
            VIN_Missing=list(set(df['id'])-set(df['url']))
...
def clean_bodytype(df):
    ...
    #print('Inital bodystyles from decoded data is : ', set(df['Body Class']))
    ...
    logger.info(df['Vehicle_information.body_style'].unique())
    logger.info(f"size is  {df.shape}")
...
def clean_drivetrain(df):
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace='Rear Wheel Drive', value='RWD', regex=True, inplace=True)
...
def clean_condition_price(df):
    ...
    df.loc[df[((df['condition'].isnull())|(df['condition'].isin(['Family;Fully Loaded', 'Clean']))) & (df['kilometers'] >= 15000) & (df['year']< today.year-1)].index,'condition'] = 'Used'
    df=df[df['condition'].isin(['New', 'Used', 'Certified Pre-Owned'])]
    df = df.reset_index(drop=True)
...
#     df.loc[df[(df['condition'].isnull()) & (df['kilometers'] >= 15000) & (df['year']< today.year)].index,'condition'] = 'Used'
    ...
    #print("Final conditions:", set(df['new=0, certified=1, used=2']))
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/canadadrives/canadadrives_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/canadadrives/decoded.csv"
    ...
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/canadadrives/cleaned_canadadrives_timeless.csv"
    EXPORT_PATH_TIME = "MasterCode1/cleaning/canadadrives/cleaned_canadadrives_time.csv"
   ...
   #df, decoder = get_decoder(df, MODULE_FULL_PATH, DECODED_PATH)
