...
logger_name = 'cargurus_cleaner_bots_canada'
email_subject = 'CarGurus Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df['make']=df.title.copy()
    df['model']=df.title.copy()
    df['Vehicle_information.trim']=df.title.copy()
    df.rename(columns={'state':"province", "_id":"id"}, inplace=True)
...
def initial_formatting(df, dfcleaned):
    ...
    df['price'].replace(to_replace=r'\$', value='', regex=True, inplace=True)
    df['price'].replace(to_replace=r'\(No-haggle price\)', value='', regex=True, inplace=True)
...
def clean_makes(df,trims_kbb):
    ...
    df['make'] = [x[1] for x in df['make'].str.split()]
...
def clean_numowners_previousaccident(df):
    ...
    logger.info(f"Cleaning number of owners and previous accidents...")
    ...
    logger.info(f"Cleaning number of owners and previous accidents... Done")
    logger.info(df['new=0, certified=1, used=2'].unique())
    logger.info(f"size is  {df.shape}")
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/cargurusCA/cargurusCA_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/cargurusCA/cargurusCA_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/cargurusCA/decoded.csv"
    ...
    #PROGRESS_PATH = 'MasterCode1/cleaning/cargurusCA/cleaned_cargurusCA_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/cargurusCA/cleaned_cargurusCA_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/cargurusCA/cleaned_cargurusCA_time.csv'
