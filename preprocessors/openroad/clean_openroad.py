...
logger_name = 'openroad_cleaner_bots_canada'
email_subject = 'Openroad Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = {"_id": "id", 'scraped_first_time':'date_added', '_VIN':'vin'}, inplace = True)
...
def clean_fueltype(df):
    ...
    df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Electric/Gas.*', value='Hybrid', regex=True, inplace=True)
...
def clean_bodytype(df):
    ...
    #print('Initial bodystyles from decoded data is : ', set(df['Body Class']))
    ...
    df['Vehicle_information.body_style'].replace(to_replace=r'.*Roadster.*', value='Convertible', regex=True, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='Cabriolet', value='Convertible', regex=True, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace=r'.*Cabriolet.*', value='Convertible', regex=True, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace=r'.*Convertible.*', value='Convertible', regex=True, inplace=True)
...
def clean_drivetrain(df):
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace='4Wd', value='4WD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Awd', value='AWD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Fwd', value='FWD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Rwd', value='RWD', regex=True, inplace=True)
...
#     df.loc[df[(df['condition'].isnull()) & (df['kilometers'] >= 15000) & (df['year']< today.year)].index,'condition'] = 'Used'
    df['new=0, certified=1, used=2'] = df['condition'].replace({'New':0, 'Demo':0, 'Used':2, 'Certified Used':1})
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/openroad/openroad_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/openroad/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/openroad/cleaned_openroad_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/openroad/cleaned_openroad_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/openroad/cleaned_openroad_time.csv'
