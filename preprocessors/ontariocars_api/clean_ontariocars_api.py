...
logger_name = 'ontariocars_cleaner_bots_canada'
email_subject = 'Ontariocars Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns={ 'scraped_first_time':'date_added'}, inplace=True)
...
def initial_formatting(df, dfcleaned):
    ...
    df['Vehicle_information.trim']=[ast.literal_eval(str(df.loc[x, 'meta']))['Trim'] if (str(df.loc[x, 'meta'])!='nan'and 'Trim' in str(df.loc[x, 'meta'])) else df.loc[x, 'title'] for x in df.index]
    ...
    df['kilometers'].replace(to_replace=r'-', value='', regex=True, inplace=True)
    df['kilometers'].replace(to_replace=r'.*https.*', value='', regex=True, inplace=True)
    df['kilometers'].replace(to_replace=r'kilometers', value='', regex=True, inplace=True)
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/ontariocars/ontariocars_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/ontariocars/ontariocars_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/ontariocars/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/ontariocars/cleaned_ontariocars_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/ontariocars/cleaned_ontariocars_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/ontariocars/cleaned_ontariocars_time.csv'
