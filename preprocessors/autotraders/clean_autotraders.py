...
logger_name = 'autotrader_cleaner_bots_canada'
email_subject = 'Autotrader Cleaner Bot Alert'
...
def import_files(RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df=df[['date_added', 'date_removed', 'id', 'title', 'url', 'make', 'model',
...
def initial_formatting(df, dfcleaned):
    ...
    if dfcleaned.empty:
        ...
        logger.info(f"Romoving cleaned data")
        logger.info(f"size is  {df.shape}")
    ...
    df.drop(df[df['kilometers'].astype(str).str.contains('address|Not Available', na=False)].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    ...
    #Corrects year
    # # =============================================================================
    df.drop(df[df['year'].astype(str).str.contains('New', na=False)].index,inplace=True)
    ...
    logger.info(f"df after formatting")
    logger.info(df)
    logger.info(f"size is  {df.shape}")
        ...
        if this_model_to_keep not in (thesemodels):
            ...
            for thismodel in thesemodels:
                ...
                if m==len(thesemodels): 
                    if link !='':
                        url_model=urllib.parse.unquote(link.split(r'/')[5]).replace("-", ' ').title()
                        if url_model in thesemodels:
                            this_model_to_keep=url_model
                            ind_to_drop='No'
...
#     #print("The original list is : " + str(make_model_list))
    ...
    df[['model_to_keep','ind_to_drop']]=df[['make', 'model_to_keep', 'url' ]].copy().swifter.progress_bar(False).apply(lambda info : model_index_to_review(info), axis=1, result_type="expand")
...
# def get_decoder(df, MODULE_FULL_PATH, DECODED_PATH):
...
# df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gasoline Hybrid.*', value='Hybrid', regex=True, inplace=True)
# df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gasoline - Hybrid.*', value='Hybrid', regex=True, inplace=True)
# df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gas/Electric Hybrid.*', value='Hybrid', regex=True, inplace=True)
...
# df['Vehicle_information.fuel_type'].replace(to_replace='Flexible', value='Gas', regex=True, inplace=True)
# df['Vehicle_information.fuel_type'].replace(to_replace='Compressed Natural Gas', value='Natural Gas', inplace=True)
# df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gaseous.*', value='Natural Gas', inplace=True)
...
def clean_transmission(df):
    ...
    df['Vehicle_information.transmission'].replace(to_replace=r'.*Sequential.*', value='Manual', regex=True, inplace=True)
...
def clean_numowners_previousaccident(df):
    ...
    logger.info(f"Cleaning number of owners abd previous accidents....")
    ...
    logger.info(f"Cleaning number of owners abd previous accidents....Done")
    logger.info(f"size is  {df.shape}")
...
def last_formatting(df):
    ...
    #print(dfWO)
    logger.info(f"dfWO")
    logger.info(f"dataframe is  {dfWO}")
    ...
    #transform to datetime
        ...
        logger.info(f"dfW1")
        logger.info(f"dataframe is  {dfW1}")
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/autotraderCA/autotraderCA_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/autotraders/autotraders_result.csv'
    #DECODED_PATH = "MasterCode1/cleaning/autotraders/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/autotraderCA/cleaned_autotraderCA_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/autotraderCA/cleaned_autotraderCA_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/autotraderCA/cleaned_autotraderCA_time.csv'
