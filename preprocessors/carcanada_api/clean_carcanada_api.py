...
logger_name = 'carcanada_cleaner_bots_canada'
email_subject = 'Carcanada Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df['Engine'] = np.where(df['Vehicle_information.fuel_type'].isnull(), df['Vehicle_information.engine'], df['Vehicle_information.fuel_type'])
    df['Vehicle_information.fuel_type']=''
    df=df[['date_added', 'date_removed', 'id', 'title', 'vin', 'url', 'make', 'model',
           ...
           'Vehicle_information.fuel_type', 'Vehicle_information.engine','Vehicle_information.trim', 'img_url',
...
def initial_formatting(df, dfcleaned):
    ...
    df=df[["id", 'url','date_added',"date_removed",'title',"make","model", "year","kilometers", "price","condition","province", "City", "Vehicle_information.transmission",
...
# # =============================================================================
   ...
   'YT':'Yukon'
    ...
    #### clean province 
    df=df[df['province'].isin(list(can_province_abbrev_code.values()))]
    df.reset_index(drop=True, inplace=True)
...
def get_decoder(df, DECODED_PATH):
    ...
    #             |(df['Vehicle_information.body_style'].isnull())]
...
def clean_bodytype(df):
    ...
    # df['Vehicle_information.body_style'].replace(to_replace=r'.*Suv.*', value='Suv', regex=True, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace='Minivan / Van', value='TBD', regex=True, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace='Suv / Crossover', value='Suv', regex=True, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace='Suv \\/ Crossover', value='Suv', regex=True, inplace=True)
    ...
    # df['Vehicle_information.body_style'].replace(to_replace=r'.*Truck.*', value='Pickup', regex=True, inplace=True)
    ...
    # df['Vehicle_information.body_style'].replace(to_replace=r'.*crew.*', value='Pickup', regex=True, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace=r'.*Truck.*', value='Pickup', regex=True, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace=r'.*Cargo.*', value='Pickup', regex=True, inplace=True)
    ...
    # df['Vehicle_information.body_style'].replace(to_replace='Minivan / Van', value='TBD', regex=False, inplace=True)
    # df['Vehicle_information.body_style'].replace(to_replace='Commercial Van', value='TBD', regex=False, inplace=True)
...
def clean_drivetrain(df):
    ...
    logger.info(f"Cleaning drivtrain....")
    ...
    logger.info(f"Cleaning drivtrain....Done")
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/carcanada/carcanada_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/car_canada/decoded.csv"
    ...
    #PROGRESS_PATH = 'MasterCode1/cleaning/car_canada/cleaned_car_canada_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/car_canada/cleaned_car_canada_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/car_canada/cleaned_car_canada_time.csv'
