...
logger_name = 'kitchenerford_cleaner_bots_canada'
email_subject = 'Kitchenerford Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    # df0.rename(columns = {"_id": "id",'scraped_first_time':'date_added', 'city':'City','Condition':'condition','Vehicle_information.drive_train':"Vehicle_information.drivetrain"}, inplace = True)
    # df0['id'] = np.where(df0['vin'].isnull(), df0['url'], df0['vin'])
    # df0['id'] = df0['id'].str.strip()
    # df0['Vehicle_information.trim'] = np.where(df0['Vehicle_information.trim'].isnull(), df0['title'], df0['Vehicle_information.trim'])
    # #print(df0)
    # #print(df0.columns)
    ...
    df.rename(columns = {"_id": "id", 'scraped_first_time':'date_added', 'city':'City', 'Condition':'condition','Vehicle_information.drive_train':"Vehicle_information.drivetrain", 'state':'province'}, inplace = True)
    #print('Display recent results')
    pd.set_option('display.max_columns', None)
    df['id'] = np.where(df['vin'].isnull(), df['url'], df['vin'])
    df['id'] = df['id'].str.strip()
    df['Vehicle_information.trim'] = np.where(df['Vehicle_information.trim'].isnull(), df['title'], df['Vehicle_information.trim'])
...
def initial_formatting(df, dfcleaned):
    ...
    df['kilometers'].replace(to_replace=r'\n', value='', regex=True, inplace=True)
    df['kilometers'].replace(to_replace=r'N/A', value='', regex=True, inplace=True)
    df['kilometers'].replace(to_replace=r'                                 ', value='', regex=True, inplace=True)

    df.drop(df[(df["kilometers"].isnull())|(df["kilometers"]=="")|(df["kilometers"]=="nan")|(df['kilometers'].str.contains('\$'))].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    df['kilometers'] = df['kilometers'].astype(int)

    #print('kilometers cleaned')
    #print(df)
    ...
    df['price'].replace(to_replace=r'\$', value='', regex=True, inplace=True)
    df['price'].replace(to_replace=r'\n', value='', regex=True, inplace=True)
    df.drop(df[(df["price"].isnull())|(df["price"]=="")|(df["price"]=="CALL")].index,inplace=True)
...
def get_decoder(df, DECODED_PATH):
    ...
    #             |(df['Vehicle_information.body_style'].isnull())|(df['Vehicle_information.body_style'].str.contains('Other|Unsure|Commercial', na=False))]
    result=df.copy()
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = "MasterCode1/cleaning/kitchenerford/kitchenerford_result_previous.csv"
    RESULT_PATH = "MasterCode1/scraping/kitchenerford/kitchenerford_result.csv"
    DECODED_PATH = "MasterCode1/cleaning/kitchenerford/decoded.csv"
    ...
    # PROGRESS_PATH = 'MasterCode1/cleaning/kitchenerford/cleaned_kitchenerford_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/kitchenerford/cleaned_kitchenerford_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/kitchenerford/cleaned_kitchenerford_time.csv'
