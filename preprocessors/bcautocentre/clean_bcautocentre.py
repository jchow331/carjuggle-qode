...
logger_name = 'bcautocentre_cleaner_bots_canada'
email_subject = 'bcautocentre Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    # df0['id']=np.where(df0['url'].isnull(), df0['stock_num'], df0['url'])
    ...
    pd.set_option('display.max_columns', None)
    df.rename(columns={"_VIN": "vin"}, inplace=True)
...
def initial_formatting(df, dfcleaned):
    ...
    df=df[["id", 'url', 'vin', 'date_added',"date_removed","make","model", "year","kilometers", "price","condition","province", "City", "Vehicle_information.transmission",
    ...
    df.drop(df[(df["price"].isnull())|(df["price"]=="")|(df["price"]=='Call for price')].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
...
def clean_makes(df,trims_kbb):
    ...
    df['make']=df['make'].str.split().str[1]
...
#             print(' I entered')
    ...
    if thismake=='Mercedes Benz':
        if 'Class' not in str(new_model):
            new_model= new_model + ' Class'
...
def difference(string1, string2):
    ...
    ## string1 had to be the one with more words
    # Split both strings into list items
    string1 = string1.split()
    string2 = string2.split()
 
    str_diff = [x for x in string1 if x not in string2]
    isEmpty = (len(str_diff) == 0)
 
    if isEmpty:
        return ''
    else:
        return listToString(list(str_diff))
...
def clean_models(df, trims_kbb):
    ...
    df['mm']= df.year.astype(str).str.cat(df.make.astype(str), sep=' ')
    df['model'] = df.apply(lambda x: difference(x['model'], x['mm']), axis=1)
...
def clean_trims(df, trims_kbb):
    ...
    df['Vehicle_information.trim'] = df.apply(lambda x: difference(x['Vehicle_information.trim'], x['mm']), axis=1)
    df['Vehicle_information.trim'] = df.apply(lambda x: x['Vehicle_information.trim'].replace(x['model'], ""), axis=1)
...
def is_province(info):
   ...
   'YT':'Yukon'
    ...
    if province.strip().upper() in list(can_province_abbrev_code.keys()):
        ...
        ind_to_drop = 'keep'
    elif province.strip().title() in list(can_province_abbrev_code.values()):
        ind_to_drop = 'keep'
    else:
        province=''
        ind_to_drop = index
...
# # =============================================================================
    ...
    # df['province'].replace(to_replace='Ontario', value='ON', regex=True, inplace=True)
...
def clean_bodytype(df):
    ...
    df['Vehicle_information.body_style'] = df['Vehicle_information.body_style'].astype(str)
...
def clean_drivetrain(df):
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace='AWD 4MATIC', value='AWD', regex=True, inplace=True)
...
def clean_colours(df):
    ...
    df['Vehicle_information.exterior_colour'].replace(to_replace=r'.*Charbon.*', value='Grey', regex=True, inplace=True)
...
def clean_numowners_previousaccident(df):
    ...
    logger.info(f"Cleaning cowners and accidents...")
    ...
    #print(df.shape)
    logger.info(f"Cleaning cowners and accidents... Done")
    logger.info(f"size is  {df.shape}")
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/bcautocentre/bcautocentre_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/bcautocentre/bcautocentre_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/bcautocentre/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/bcautocentre/cleaned_bcautocentre_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/bcautocentre/cleaned_bcautocentre_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/bcautocentre/cleaned_bcautocentre_time.csv'
    ...
    # df, decoder = get_decoder(df, DECODED_PATH)
    #export_files(geoloc, extra_geo, GEOLOC_PATH, EXTRA_GEO_PATH)
