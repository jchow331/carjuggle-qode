...
logger_name = 'shopwilson_cleaner_bots_canada'
email_subject = 'Shopwilson Cleaner Bot Alert'
...
def import_files(RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = {"_id": "id", 'scraped_first_time':'date_added', 'city':'City'}, inplace = True)
    #print('Display recent results')
    pd.set_option('display.max_columns', None)
    df['id'] = np.where(df['vin'].isnull(), df['url'], df['vin'])
    df['id'] = df['id'].str.strip()
    if "Vehicle_information.drivetrain" not in df.columns:
        df["Vehicle_information.drivetrain"] = ''
    df=df[['date_added', 'date_removed', 'id', 'vin', 'url', 'make', 'model',
           ...
           'Vehicle_information.fuel_type', 'Vehicle_information.trim',
...
def initial_formatting(df, dfcleaned):
    ...
    df.drop(df[(df["price"].isnull())|(df["price"]=="")|(df["price"]=="Contact Us For Price")].index,inplace=True)
    ...
    df.drop(df[df['kilometers'].astype(str).str.contains('address', na=False)].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    ...
    df.drop(df[(df["price"]=="<span class=\\")].index,inplace=True)
...
def clean_makes(df,trims_kbb):
   ...
   # Fix makes using fix_truncated_makes function
    ...
    df['make'] = df['make'].swifter.progress_bar(False).progress_bar(False).apply(lambda x : fix_truncated_makes(x))
...
######## equivalent to:
            ...
            X['model'].str.split().swifter.progress_bar(False).progress_bar(False).apply(collection_words_temp_model.update)
...
def clean_models(df, trims_kbb):
    ...
    df['model_to_keep']=df[['year','make', 'model' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : remove_extra_words_from_models(info), axis=1)
...
#     #print("The original list is : " + str(make_model_list))
    ...
    df[['model_to_keep','ind_to_drop']]=df[['make', 'model_to_keep' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : model_index_to_review(info), axis=1, result_type="expand")
...
#     #print(models_to_be_removed)
    ...
    new_df[['model_to_keep','Vehicle_information.trim','ind_to_drop']] = df.loc[ind_to_drop,('make','model_to_keep' ,'Vehicle_information.trim')].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : model_last_try(info), axis=1, result_type="expand")
...
def remove_extra_words_from_trims(info):
    ...
    if (str(df_Year)!='nan') & (str(df_Make)!='nan') & (str(df_Model)!='nan') & (str(df_Trim)!='nan'):
        ...
        if X.empty:
            ...
            X['trim_mod'].str.split().swifter.progress_bar(False).progress_bar(False).apply(collection_words_temp.update)
            X['trim1'].str.split().swifter.progress_bar(False).progress_bar(False).apply(collection_words_temp.update)
...
def clean_trims(df, trims_kbb):
    ...
    df['trim_to_match']=df[['year','make', 'model_to_keep', 'Vehicle_information.trim' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : remove_extra_words_from_trims(info), axis=1)

    #print(df)

    #print("Trim: cheking indexes to try again")
    #tqdm_notebook().pandas(desc="Trim: cheking indexes to try again")
    df[['trim_to_keep','ind_to_drop']]=df[['year','make', 'model_to_keep', 'Vehicle_information.trim' ,'trim_to_match']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : trim_index_to_review(info), axis=1, result_type="expand")
    ##print(df[['year','make', 'model_to_keep', 'Vehicle_information.trim' ,'trim_to_match']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : trim_index_to_review(info), axis=1, result_type="expand"))
    #print(df)
...
#     #print(models_to_be_removed)
    ...
    new_df=pd.DataFrame()
    new_df[['trim_to_keep','ind_to_drop']] = df.loc[ind_to_drop,('year','make','model_to_keep' ,'trim_to_match')].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : trim_last_try(info), axis=1, result_type="expand")
    df.update(new_df)
...
# # =============================================================================
   ...
   'YT':'Yukon'}
    ...
    df['province']=df['province'].replace(can_province_abbrev_code)
...
def clean_cities(df):
    ...
    df[['City2','ind_to_drop']] = df[['City', 'province']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : match(info), axis=1, result_type="expand")
    ...
    if ind_to_drop!=[]:
        ...
        if list_tuples==[]:
            ...
            df_result[['City2','match2']] = df_result[['city_to_check1', 'province']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : match(info), axis=1, result_type="expand")


            df_result[['City3','match3']] = df_result[['city_to_check2', 'province']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : match(info), axis=1, result_type="expand")


            df_result[['City4','match4']] = df_result[['city_name', 'province']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : match(info), axis=1, result_type="expand")


            #print(df_result)

            df_result['City2'] = df_result.swifter.progress_bar(False).progress_bar(False).apply(lambda x: x['City2'] if x['City3']=='' else x['City3'], axis=1)
            df_result['City2'] = df_result.swifter.progress_bar(False).progress_bar(False).apply(lambda x: x['City2'] if x['City4']=='' else x['City4'], axis=1) 
            ...
            df_result[['City2', 'index_to_drop']]= df_result[['City', 'province']].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : scrape_url_geogratis(info), axis=1, result_type="expand")
...
def get_decoder(df, DECODED_PATH):
    ...
    logger.info(f"Decoding vins")

    aws = AWSHandler()
    
    # only decode the observation with missing values in transmission, bodystyle, fueltype and drivetrain
    # result = df[(df['Vehicle_information.transmission'].astype(str).str.strip()=='')|(df['Vehicle_information.transmission'].astype(str).str.strip()=='-')
    #             |(df['Vehicle_information.transmission'].isnull())|(df['Vehicle_information.transmission'].astype(str).str.contains('Other|Unsure', na=False))
    #             |(df['Vehicle_information.drivetrain'].astype(str).str.strip()=='')|(df['Vehicle_information.drivetrain'].astype(str).str.strip()=='-')
    #             |(df['Vehicle_information.drivetrain'].isnull())|(df['Vehicle_information.drivetrain'].astype(str).str.contains('Other|Unsure', na=False))
    #             |(df['Vehicle_information.fuel_type'].astype(str).str.strip()=='')|(df['Vehicle_information.fuel_type'].astype(str).str.strip()=='-')
    #             |(df['Vehicle_information.fuel_type'].isnull())|(df['Vehicle_information.fuel_type'].astype(str).str.contains('Other|Unsure', na=False))
    #             |(df['Vehicle_information.body_style'].astype(str).str.strip()=='')|(df['Vehicle_information.body_style'].str.strip()=='')
    #             |(df['Vehicle_information.body_style'].isnull())|(df['Vehicle_information.body_style'].astype(str).str.contains('Other|Unsure', na=False))]
    ...
    try:
        ...
        logger.info(f"VINS are missing  {len(VIN_Missing)}")
    except:
        try:
    ...
    if len(VIN_Missing) == 0:
        ...
        for vin in VIN_Missing:
            k=k+1
            if str(vin)=='nan' or ' ' in vin or 'http' in vin or len(str(vin).strip())!=17:
                pass
            else:
                try:
...
def clean_fueltype(df):
    ...
    df[['Vehicle_information.fuel_type','ind_to_drop']]=df[['year','make', 'model_to_keep', 'trim_to_keep' ,'Vehicle_information.fuel_type' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : lookup_fueltype(info), axis=1, result_type="expand")
    #print(df)
    ...
    new_df=pd.DataFrame()
    new_df['Vehicle_information.fuel_type'] = df.loc[fuel_to_drop,'Fuel Type - Primary' ].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : fueltype_last_try(info), axis=1)
    #print(new_df)                                                                                                          
    df.update(new_df)
...
def clean_bodytype(df):
    ...
    logger.info(f"Cleaning bodystyle...")
    ...
    df['Vehicle_information.body_style'].replace(to_replace='Cpe', value='Coupe', regex=True, inplace=True)
    ...
    df['Vehicle_information.body_style'].replace(to_replace='Suvs', value='Suv', regex=True, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='Truck', value='Pickup', regex=True, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='Trucks', value='Pickup', regex=True, inplace=True)
    ...
    df['Vehicle_information.body_style'].replace(to_replace='Vans', value='TBD', regex=False, inplace=True)
    ...
    df[['body_style_to_keep','ind_to_drop']]=df[['year','make', 'model_to_keep', 'trim_to_keep' ,'Vehicle_information.body_style' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : lookup_bodystyle(info), axis=1, result_type="expand")
    #print(df)
    ...
    new_df=pd.DataFrame()
    new_df['body_style_to_keep'] = df.loc[style_to_drop,'Body Class' ].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : bodystyle_last_try(info), axis=1)
    ...
    logger.info(f"Cleaning bodystyle....Done")
...
def clean_drivetrain(df):
    ...
    #print ("Drivetrain initial set: ", df['Drive Type'].unique())
    ...
    #print ("Drivetrain initial set: ", df['Vehicle_information.drivetrain'].unique())
    
    df['Vehicle_information.drivetrain'] = df['Vehicle_information.drivetrain'].astype(str).str.strip()
    df['Vehicle_information.drivetrain'].replace(to_replace='All-Wheel Drive', value='AWD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Four-Wheel Drive', value='4WD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Rear-Wheel Drive', value='FWD', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='Front-Wheel Drive', value='FWD', regex=True, inplace=True)
    ...
    df[['Vehicle_information.drivetrain','ind_to_drop']]=df[['year','make', 'model_to_keep', 'trim_to_keep' ,'Vehicle_information.fuel_type','body_style_to_keep','Vehicle_information.drivetrain' ]].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : lookup_drivetrain(info), axis=1, result_type="expand")
    #print(df)
    ...
    new_df=pd.DataFrame()
    new_df['Vehicle_information.drivetrain'] = df.loc[drivetrain_to_drop,'Drive Type' ].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : drivetrain_last_try(info), axis=1)
    #print('new_df, check indexes' )
    #print(new_df)
    df.update(new_df)
...
def clean_transmission(df):
    ...
    df['Vehicle_information.transmission'].replace(to_replace='Auto-Shift Manual w/OD', value='Automatic', regex=True, inplace=True)
    ...
    new_df=pd.DataFrame()
    new_df['Vehicle_information.transmission'] = df.loc[transmission_to_drop,'Transmission Style' ].copy().swifter.progress_bar(False).progress_bar(False).apply(lambda info : transmission_last_try(info), axis=1)
    #print('new_df, check indexes' )
    #print(new_df)
    df.update(new_df)
...
def clean_condition_price(df):
    ...
    logger.info(f"Cleaning condition...")
...
#     df.loc[df[(df['condition'].isnull()) & (df['kilometers'] >= 15000) & (df['year']< today.year)].index,'condition'] = 'Used'
    ...
    #print(df.shape)
    logger.info(f"Cleaning condition....Done")
...
def clean_numowners_previousaccident(df):
    ...
    logger.info(f"Cleaning accidents and owners...")
    ...
    #print(df.shape)
    logger.info(f"Cleaning accidents and owners....Done")
    logger.info(f"size is  {df.shape}")
...
def last_formatting(df):
    ...
    dfW1.drop((dfW1[dfW1['date_removed'] == 'None'].index), inplace=True)
    dfW1.reset_index(inplace=True, drop=True)

    dfW1.drop((dfW1[dfW1['date_added'] == ''].index), inplace=True)
    dfW1.drop((dfW1[dfW1['date_added'] == 'None'].index), inplace=True)
    ...
    #transform to datetime
        dfW1['date_added'] = pd.to_datetime(dfW1['date_added'], utc=True, errors='coerce')
        dfW1['date_added'] = dfW1['date_added'].dt.date
        dfW1['date_removed'] = pd.to_datetime(dfW1['date_removed'], utc=True, errors='coerce')
        dfW1['date_removed'] = dfW1['date_removed'].dt.date
        #calculate duration
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/shopwilsons/shopwilson_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/shopwilsons/decoded.csv"
    ...
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/shopwilsons/cleaned_shopwilsons_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/shopwilsons/cleaned_shopwilsons_time.csv'
