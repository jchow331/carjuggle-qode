...
#import concurrent.futures as futuresls
import requests
...
logger_name = 'vinn_auto_cleaner_bots_canada'
email_subject = 'Vinn Auto Cleaner Bot Alert'
...
def import_files(RESULT_PATH,EXPORT_PATH_TIMELESS, KBB_PATH, LOCATIONS_PATH, GEOLOC_PATH , EXTRA_GEO_PATH):
    ...
    # df0['id']=np.where(df0['vin'].isnull(), df0['url'], df0['vin'])
    # df0['id']=df0['id'].str.strip()
    # #print(df0)
    # #print(df0.columns)

    # list_paths= get_list_paths(BUCKET, PREFIX)
    # list_paths.remove('scrapers/vinn_auto/results/vinn_auto_2022-01-21.csv')
    # list_dfs=[]
    # for RESULT_PATH in list_paths:
    ...
    # df1['city']=df1['url'].str.split('/').str[-2].str.title()
    df['City']=np.where(df['city'].isnull(), df['url'].str.split('/').str[-2].str.title(), df['city'].str.strip())
    ...
    # df1=pd.concat(list_dfs, axis=0, sort=False)
    ...
    logger.info(f"Concatenate results{df}")
    ...
    logger.info(f"Cleaned past results{dfcleaned}")
    ...
    logger.info(f"locations file {locations}")
    ...
    logger.info(f"geoloc file {geoloc}")
    ...
    logger.info(f"extra_geo file {extra_geo}")
...
def initial_formatting(df, dfcleaned):
    ...
    # Initial formatting
    # =============================================================================
    logger.info(f'Initial fomatting and cleaning ids, prices, kilometers, years, condition .......')
    ...
    if dfcleaned.empty:
        ...
        logger.info(f"dRomoving cleaned data {df.shape}")
    ...
    pd.set_option('display.max_columns', None)
    logger.info(f"Initial formatting {df}")
    logger.info(df.info())
...
def clean_makes(df,trims_kbb):
    ...
    logger.info("Fix makes using fix_truncated_makes function: ... ")
   ...
   # Fix makes using fix_truncated_makes function
    ...
    logger.info("Cleaning makes: Done.")
    # #print ("size", df.shape)
...
def clean_models(df, trims_kbb):
    ...
    logger.info(f"Cleaning models: ...(4/11) ")
    ...
    logger.info(f"Model: Removing extra words")
    ...
    logger.info(f"df size {df,df.shape }")
...
def clean_trims(df, trims_kbb):
    ...
    logger.info(f"Cleaning trims... (5/11) ")
    df['together']= df.Series.astype(str).str.cat(df.Trim.astype(str), sep=' ')
    df['together'] = df['together'].str.title()
    df['Vehicle_information.trim'] = np.where(df['Vehicle_information.trim'].isnull(), df['together'], df['Vehicle_information.trim'])
    ...
    logger.info(f"Trim: Removing extra words")
    ...
    logger.info(f"Trim: cheking indexes to try again")
...
def clean_cities(df):
    ...
    if ind_to_drop!=[]:
        ...
        logger.info('Cleaning cities. Done')
        logger.info(df)
        logger.info(df.shape)
...
#     result = df[(df['Vehicle_information.transmission'].str.strip()=='')|(df['Vehicle_information.transmission'].str.strip()=='-')|(df['Vehicle_information.transmission'].isnull())
#    |(df['Vehicle_information.drivetrain'].str.strip()=='')|(df['Vehicle_information.drivetrain'].str.strip()=='-')|(df['Vehicle_information.drivetrain'].isnull())
#    |(df['Vehicle_information.fuel_type'].str.strip()=='')|(df['Vehicle_information.fuel_type'].str.strip()=='-')|(df['Vehicle_information.fuel_type'].isnull())
#    |(df['Vehicle_information.body_style'].str.strip()=='')|(df['Vehicle_information.body_style'].str.strip()=='')|(df['Vehicle_information.body_style'].isnull())
#    |(df['Vehicle_information.trim'].str.strip()=='')|(df['Vehicle_information.trim'].str.strip()=='')|(df['Vehicle_information.trim'].isnull())]
    ...
    try:
        ...
        logger.info('1_VINS are missing')
        logger.info(len(VIN_Missing))

    except:
        try:
            decoder=aws.download_object_as_csv(DECODED_PATH)
            VIN_Missing=list(set(result['id'])-set(decoder['vin'])-set(result['url']))
            logger.info('2_VINS are missing')
            logger.info(len(VIN_Missing))
        except:
            VIN_Missing=list(set(result['id'])-set(result['url']))
            decoder=pd.DataFrame()
            logger.info('3_VINS are missing')
            logger.info(len(VIN_Missing))
    ...
    for vin in VIN_Missing:
        ...
        if k%500==0 and k!=0:
            ...
            logger.info(round(k*100/len(VIN_Missing)))
...
def clean_fueltype(df):
    ...
    #print ("size", df.shape)
    logger.info("Cleaning Fuel Style: Done.\n")
    logger.info("Final fuel Type set")
...
#     df.loc[df[(df['condition'].isnull()) & (df['kilometers'] >= 15000) & (df['year']< today.year)].index,'condition'] = 'Used'
    df['new=0, certified=1, used=2'] = df['condition'].replace({'New':0, 'Used':2, 'Certified':1})
...
def clean_numowners_previousaccident(df):
    logger.info(f"Cleaning owners/accidents... ")
...
def export_files(DF1, link1):
    ...
    try:
        ...
        logger.info(f"previous df size... {dfcleaned_timeless.shape}")
        ...
        logger.info(f"new df size... {df.shape}")
    except:
        aws.upload_csv_object(DF1, link1)
        #print(DF1.shape)
        logger.info(f"new df size... {DF1.shape}")

    
# def get_list_paths(BUCKET, PREFIX):
#     aws_access_key_id=''
#     aws_secret_access_key=''
 
#     s3=boto3.client('s3', aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
#     my_bucket_objects=s3.list_objects(Bucket=BUCKET,Prefix=PREFIX)
...
def main():
    ...
    # BUCKET = 'carjuggle-data-science'
    # PREFIX = 'scrapers/vinn_auto/results'

    #PREVIOUS_RESULT_PATH = 'MasterCode1/scraping/vinnauto/vinnauto_result.csv'
    # RESULT_PATH = get_result_path('MasterCode1/scraping/vinnauto/', 'vinn_auto')
    # logger.info(RESULT_PATH)
    RESULT_PATH = 'MasterCode1/scraping/vinnauto/vinnauto_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/vinnauto/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/vinnauto/cleaned_vinnauto_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/vinnauto/cleaned_vinnauto_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/vinnauto/cleaned_vinnauto_time.csv'
    #WEBDRIVER_PATH = '/Users/SanaKrichen/Downloads/chromedriver'
    
    global trims_kbb
    global locations
    global extra_geo
    global geoloc
    df, dfcleaned, trims_kbb, locations, geoloc, extra_geo = import_files( RESULT_PATH, EXPORT_PATH_TIMELESS, KBB_PATH, LOCATIONS_PATH, GEOLOC_PATH , EXTRA_GEO_PATH)
