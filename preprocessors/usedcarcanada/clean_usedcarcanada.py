...
logger_name = 'usedcaracanada_cleaner_bots_canada'
email_subject = 'UsedCarCanada Cleaner Bot Alert'
...
def import_files(RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df=df[['id', 'url','date_added', 'date_removed', 'make', 'model',
...
def initial_formatting(df, dfcleaned):
    ...
    #print('cleaning kilometers', df.shape)
...
def get_decoder(df, DECODED_PATH):
    ...
    try:
        ...
        logger.info(f"len(VIN_Missing  {len(VIN_Missing)}")
    except:
        try:
...
def clean_fueltype(df):
    ...
    df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Propane.*', value='Gas', regex=True, inplace=True)
...
def clean_bodytype(df):
    ...
    df['Vehicle_information.body_style'].replace(to_replace='Minivan/Van', value='TBD', regex=False, inplace=True)
...
def lookup_drivetrain(info):
    ...
    possible_drivetrain=list(set(trims_kbb[(trims_kbb['year']==year) & (trims_kbb['make']==make) & (trims_kbb['model']==model_to_keep) & (trims_kbb['trim_mod']==trim_to_keep)& (trims_kbb['fuel_type']==fueltype)& (trims_kbb['body_style']==bodystyle_to_keep)& (trims_kbb['transmission']==transmission)]['drive_train']))
...
def clean_drivetrain(df):
    ...
    df[['Vehicle_information.drivetrain','ind_to_drop']]=df[['year','make', 'model_to_keep', 'trim_to_keep' ,'Vehicle_information.fuel_type','body_style_to_keep','Vehicle_information.transmission','Vehicle_information.drivetrain' ]].copy().swifter.progress_bar(False).apply(lambda info : lookup_drivetrain(info), axis=1, result_type="expand")
    #print(df)
...
def lookup_transmission(info):
    ...
    possible_transmission=list(set(trims_kbb[(trims_kbb['year']==year) & (trims_kbb['make']==make) & (trims_kbb['model']==model_to_keep) & (trims_kbb['trim_mod']==trim_to_keep)& (trims_kbb['fuel_type']==fueltype)& (trims_kbb['body_style']==bodystyle_to_keep)]['transmission']))
...
def clean_transmission(df):
    ...
    df['Vehicle_information.transmission'].replace(to_replace='Auto', value='Automatic', regex=True, inplace=True)
    ...
    df[['Vehicle_information.transmission','ind_to_drop']]=df[['year','make', 'model_to_keep', 'trim_to_keep' ,'Vehicle_information.fuel_type','body_style_to_keep', 'Vehicle_information.transmission' ]].copy().swifter.progress_bar(False).apply(lambda info : lookup_transmission(info), axis=1, result_type="expand")
    #print(df)
...
#     df.loc[df[(df['condition'].isnull()) & (df['kilometers'] >= 15000) & (df['year']< today.year)].index,'condition'] = 'Used'
    df['new=0, certified=1, used=2'] = df['condition'].replace({'condition-new':0, 'condition-used':2, 'condition-certified':1})
...
def main():
    ...
    #RECOVERED_RESULT_PATH = 'MasterCode1/cleaning/carpages/carpages_result_recovered.csv'
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/carpages/carpages_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/usedcarcanada/usedcarcanada_result.csv'
    #DECODED_PATH = "MasterCode1/cleaning/carpages/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/used_carcanada/cleaned_used_carcanada_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/used_carcanada/cleaned_used_carcanada_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/used_carcanada/cleaned_used_carcanada_time.csv'
