...
logger_name = 'goauto_cleaner_bots_canada'
email_subject = 'Goauto Cleaner Bot Alert'
...
def import_files(RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df['vin']=[ast.literal_eval(str(df.loc[x, 'metadata']))['vin_number'] if (str(df.loc[x, 'metadata'])!='nan'and 'vin_number' in str(df.loc[x, 'metadata'])) else df.loc[x, 'vin'] for x in df.index]
    ...
    logger.info(f"Initial import... Done")
    logger.info(df)
    logger.info(df.info())
...
def initial_formatting(df, dfcleaned):
    ...
    df=df[["url","id",'date_added',"date_removed","make","model", "year","kilometers", "price","condition","province", "City", "Vehicle_information.transmission",
       "Vehicle_information.drivetrain", "Vehicle_information.body_style", "Vehicle_information.exterior_colour",
       "Vehicle_information.fuel_type", "Vehicle_information.trim", "metadata", 'price_history']]
    ...
    logger.info(f"Cleaning kilometers")
    ...
    df.drop(df[(df["price"].isnull())|(df["price"]=="")].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    df['price'] = pd.to_numeric(df['price']).astype(float).astype(int)



    #Corrects year
    # # =============================================================================
    logger.info(f"Cleaning model year")
    df.drop(df[(df["year"].isnull())|(df["year"]=="")].index,inplace=True)
    df.reset_index(drop=True, inplace=True)
    df['year'].replace(to_replace=r'\}', value='', regex=True, inplace=True)
    df['year'].replace(to_replace=r'\{', value='', regex=True, inplace=True)
    df['year'] = pd.to_numeric(df['year']).astype(float).astype(int)
...
# # =============================================================================
    ...
    df['province'].replace(to_replace=r'.*Columbia.*', value='British Columbia', regex=True, inplace=True)
    df['province'].replace(to_replace=r'.*Territories.*', value='Northwest Territories', regex=True, inplace=True)
...
def get_decoder(df, DECODED_PATH):
    ...
    try:
        ...
        logger.info(f"1_VINS are missing  {len(VIN_Missing)}")
    except:
        try:
            decoder=aws.download_object_as_csv(DECODED_PATH)
            VIN_Missing=list(set(result['id'])-set(decoder['vin'])-set(result['url']))
            logger.info(f"2_VINS are missing  {len(VIN_Missing)}")
        except:
            VIN_Missing=list(set(result['id'])-set(result['url']))
            decoder=pd.DataFrame(columns=['vin', 'Series2', 'Series','Trim'])
            logger.info(f"3_VINS are missing  {len(VIN_Missing)}")
...
def clean_fueltype(df):
    ...
    df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gas/Electric.*', value='Hybrid', regex=True, inplace=True)
    ...
    df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gas/Ethanol.*', value='Gas', regex=True, inplace=True)
    ...
    df['Vehicle_information.fuel_type'].replace(to_replace=r'.*Gas.*', value='Gas', regex=True, inplace=True)
...
def clean_bodytype(df):
    ...
    logger.info(f"Cleaning body type... ")
    ...
    df['Vehicle_information.body_style'].replace(to_replace='-', value=' ', regex=True, inplace=True)
    ...
    df['Vehicle_information.body_style'].replace(to_replace='Passenger Van', value='TBD', regex=False, inplace=True)
    ...
    df['Vehicle_information.body_style'].replace(to_replace='5 Door', value='', regex=False, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='4 Door', value='', regex=False, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='2 Door', value='', regex=False, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace='2dr', value='', regex=False, inplace=True)
    ...
    logger.info(f"Cleaning body type... Done")
...
def clean_drivetrain(df):
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace='-', value=' ', regex=True, inplace=True)
    df['Vehicle_information.drivetrain'].replace(to_replace='All Wheel', value='AWD', regex=True, inplace=True)
    ...
    df['Vehicle_information.drivetrain'].replace(to_replace='4 Wheel Drive', value='4WD', regex=True, inplace=True)
...
def clean_transmission(df):
    ...
    df['Transmission Style'].replace(to_replace=r'.*CVT.*', value='Automatic', regex=True, inplace=True)
    df['Transmission Style'].replace(to_replace=r'.*Dual Clutch.*', value='Automatic', regex=True, inplace=True)
    df['Transmission Style'].replace(to_replace=r'.*Dual Clutch.*', value='Automatic', regex=True, inplace=True)
    #print('cleaning again')
    df['Transmission Style'].replace(to_replace=r'.*Automatic \(Automatic\).*', value='Automatic', regex=True, inplace=True)
    df['Transmission Style'].replace(to_replace=r'.*Electronic Continuously.*', value='Automatic', regex=True, inplace=True)
    ...
    df['Vehicle_information.transmission'].replace(to_replace='Continuously Variable Ratio', value='Automatic', regex=True, inplace=True)
    ...
    df['Vehicle_information.transmission'].replace(to_replace=r'.*Dual Clutch.*', value='Automatic', regex=True, inplace=True)
    df['Vehicle_information.transmission'].replace(to_replace=r'.*Manuelle.*', value='Manual', regex=True, inplace=True)
...
def clean_condition_price(df):
    ...
    df['new=0, certified=1, used=2'] = df['condition'].replace({'New':0, 'NEW':0, 'Used':2, 'USED':2, 'Certified':1})
...
def last_formatting(df):
    ...
    logger.info(f"last formatting... Done")
    logger.info(f"size is  {dfWO.shape}")
    logger.info(f"size is  {dfW1.shape}")
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/goauto/goauto_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/goauto/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/goauto/cleaned_goauto_progress.csv'
    EXPORT_PATH_TIMELESS = 'MasterCode1/cleaning/goauto/cleaned_goauto_timeless.csv'
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/goauto/cleaned_goauto_time.csv'
