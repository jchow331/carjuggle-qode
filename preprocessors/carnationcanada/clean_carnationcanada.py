...
logger_name = 'carnationcanada_cleaner_bots_canada'
email_subject = 'CarNationCanada Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = {"_id": "id", 'scraped_first_time':'date_added', 'city':'City','Vehicle_information.drive_train':'Vehicle_information.drivetrain'}, inplace = True)
    #print('Display recent results')
    pd.set_option('display.max_columns', None)
    df['id'] = np.where(df['vin'].isnull(), df['url'], df['vin'])
    df['id'] = df['id'].str.strip()
    df['Vehicle_information.trim'] = df['title'].copy()
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/carnationcanada/carnationcanada_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/carnationcanada/decoded.csv"
    ...
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/carnationcanada/cleaned_carnationcanada_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/carnationcanada/cleaned_carnationcanada_time.csv'
