...
logger_name = 'mycar_cleaner_bots_canada'
email_subject = 'Mycar Cleaner Bot Alert'
   ...
   'YT':'Yukon'
    ...
    try:
        ...
        if province.strip().upper() in list(can_province_abbrev_code.keys()):
            ...
            ind_to_drop = 'keep'
        elif province.strip().upper() in list(can_province_abbrev_code.values()):
            ind_to_drop = 'keep'
        else:
            province=''
            ind_to_drop = index
...
def clean_bodytype(df):
    ...
    df['Vehicle_information.body_style'].replace(to_replace=r'.*Van.*', value='TBD', regex=False, inplace=True)
    df['Vehicle_information.body_style'].replace(to_replace=r'.*Truck.*', value='Pickup', regex=False, inplace=True)
...
def clean_transmission(df):
    ...
    df['Vehicle_information.transmission'].replace(to_replace=r'.*Auto.*', value='Automatic', regex=True, inplace=True)
...
def main():
    ...
    #PREVIOUS_RESULT_PATH = 'MasterCode1/cleaning/mycar/mycar_result_previous.csv'
    RESULT_PATH = 'MasterCode1/scraping/mycar/mycar_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/mycar/decoded.csv"
    ...
    PROGRESS_PATH = 'MasterCode1/cleaning/mycar/cleaned_mycar_progress.csv'
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/mycar/cleaned_mycar_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/mycar/cleaned_mycar_time.csv'
