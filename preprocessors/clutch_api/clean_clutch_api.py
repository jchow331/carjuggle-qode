...
logger_name = 'clutch_cleaner_bots_canada'
email_subject = 'Clutch Cleaner Bot Alert'
...
def import_files( RESULT_PATH, KBB_PATH, LOCATIONS_PATH,GEOLOC_PATH , EXTRA_GEO_PATH, EXPORT_PATH_TIMELESS):
    ...
    df.rename(columns = {"_id": "id", 'scraped_first_time':'date_added','meta':'metadata'}, inplace = True)
    #print('Display recent results')
    pd.set_option('display.max_columns', None)
    df['id'] = np.where(df['vin'].isnull(), df['url'], df['vin'])
    df['id'] = df['id'].str.strip()
    df=df[['date_added', 'date_removed', 'id', 'vin', 'url','title', 'make', 'model',
           ...
           'price_history', 'metadata','NumOwners', 'PrevAccident']]
    df["condition"]='Used'
...
def clean_makes(df,trims_kbb):
    ...
    for i in df.index:
        df.loc[i, 'make']=df.loc[i, 'make'].replace(str(df.loc[i, 'year']), '').strip()

    df['make'] = [x[0] for x in df['make'].str.split()]
...
# # =============================================================================
    ...
    df['province'].replace(to_replace='Nova Scotia', value='NS', regex=True, inplace=True)
...
def get_decoder(df, DECODED_PATH):
    ...
    logger.info(f"Decoding vins....Done")
...
def clean_numowners_previousaccident(df):
    ...
    logger.info(f"Cleaning oweners and accidents...")
    ...
    #print(df.shape)
    logger.info(f"Cleaning oweners and accidents... Done")
    logger.info(f"size is  {df.shape}")  
...
def main():
    ...
    RESULT_PATH = 'MasterCode1/scraping/clutch/clutch_result.csv'
    DECODED_PATH = "MasterCode1/cleaning/clutch/decoded.csv"
    ...
    EXPORT_PATH_TIMELESS = "MasterCode1/cleaning/clutch/cleaned_clutch_timeless.csv"
    EXPORT_PATH_TIME = 'MasterCode1/cleaning/clutch/cleaned_clutch_time.csv'
